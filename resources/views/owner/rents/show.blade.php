<x-user-layout>
    ...
    <div class="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8" x-data>
        ...
        @php
            ...
            $isMyTurn = $rentRequest->status_id == \App\Models\Status::RNT_REQ_PENDING && $latestMsg && $latestMsg->sender_id !== Auth::id();
            $waitingForRenter = $rentRequest->status_id == \App\Models\Status::RNT_REQ_PENDING && $latestMsg && $latestMsg->sender_id === Auth::id();
        @endphp
        ...
        <div class="mb-6 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
            ...
            <div>
                ...
                @elseif($waitingForRenter)
                    <span class="bg-blue-50 text-blue-600 px-4 py-2 rounded-xl text-sm font-black border border-blue-100 tracking-wide uppercase shadow-sm">Waiting for Renter</span>
                ...
                @elseif($rentRequest->status_id == \App\Models\Status::RNT_REQ_CANCELLED)
                    <span class="bg-gray-100 text-gray-500 px-4 py-2 rounded-xl text-sm font-black border border-gray-200 tracking-wide uppercase shadow-sm">Cancelled by Renter</span>
                @else
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 xl:gap-12 items-start">
            ...
            <div class="lg:col-span-2 space-y-8">
                ...
                <div class="bg-white p-8 rounded-[2rem] border border-gray-100 shadow-sm">
                    <h3 class="text-xl font-black text-gray-900 mb-6 flex items-center gap-2"><span>💬</span> Renter's Initial Proposal</h3>
                    @php $proposalMsg = $rentRequest->messages->where('type_id', \App\Models\Status::MSG_APPLICATION)->first(); @endphp
                ...
                <div class="bg-white p-6 sm:p-8 rounded-[2rem] border border-gray-200 shadow-xl shadow-gray-100/50">
                            ...
                            <span class="text-sm font-black text-gray-900">
                                @if($rentRequest->pricing && $rentRequest->pricing->price > 0)
                                    @php
                                        ...
                                        $baseWord = match($code) { 'daily' => 'Day', 'weekly' => 'Week', 'monthly' => 'Month', default => 'Unit' };
                                    @endphp
                    ...
                    @elseif($waitingForRenter)
                        <div class="pt-6 border-t border-gray-100 text-center">
                            ...
                            <button @click.prevent="$dispatch('open-decline-modal')" class="text-xs font-bold text-red-500 hover:text-red-700 underline">Withdraw & Decline Application</button>
                        </div>
                    @endif
                </div>

                {{-- Counter-Proposal Alert Widget --}}
                @php
                    $renterProposing = $latestMsg && $latestMsg->type_id == \App\Models\Status::MSG_RESCHEDULE_PROPOSAL && $latestMsg->sender_id == $rentRequest->renter_id;
                @endphp

                @if($renterProposing && $rentRequest->status_id == \App\Models\Status::RNT_REQ_PENDING)
                    <div class="bg-blue-50 border border-blue-200 rounded-[2rem] p-6 shadow-sm">
                        ...
                        <p class="text-sm font-medium text-blue-800 mb-4 whitespace-pre-line">{{ $latestMsg->message }}</p>
                        
                        <div class="grid grid-cols-3 gap-2 mb-5 text-xs font-bold text-blue-900">
                            <div class="bg-white/60 border border-blue-100 p-2 rounded-xl text-center">Visit<br>{{ $latestMsg->reschedule->proposed_visit_date ? \Carbon\Carbon::parse($latestMsg->reschedule->proposed_visit_date)->format('M d') : 'N/A' }}</div>
                            <div class="bg-white/60 border border-blue-100 p-2 rounded-xl text-center">Start<br>{{ \Carbon\Carbon::parse($latestMsg->reschedule->proposed_start_date)->format('M d') }}</div>
                            <div class="bg-white/60 border border-blue-100 p-2 rounded-xl text-center">End<br>{{ \Carbon\Carbon::parse($latestMsg->reschedule->proposed_end_date)->format('M d') }}</div>
                        </div>

                        <div class="flex gap-2">
                            <form method="POST" action="{{ route('owner.reservations.reschedule.accept', $rentRequest->id) }}" class="w-full">
                                @csrf <button class="w-full bg-blue-600 text-white rounded-xl py-2.5 font-black text-sm hover:bg-blue-700 transition active:scale-95 shadow-sm">Accept Dates</button>
                            </form>
                            {{-- Triggers the Decline Modal! --}}
        ...
        @if($rentRequest->status_id == \App\Models\Status::RNT_REQ_PENDING)
                ...
                <div x-show="showApprove" x-transition class="fixed inset-0 z-10 w-screen overflow-y-auto">
                    <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                        <div @click.away="showApprove = false" class="relative transform overflow-hidden rounded-[2rem] bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg border border-gray-100 p-8">
                            <h3 class="text-xl font-black text-gray-900 mb-2">Accept Application</h3>
                            <form action="{{ route('owner.reservations.approve', $rentRequest->id) }}" method="POST" class="mt-6">
                                ...
                                <div>
                                    ...
                                    <textarea name="response_note" rows="3" placeholder="Hi, looking forward to meeting you!" class="w-full rounded-2xl border-gray-300 focus:border-teal-500 shadow-sm font-medium text-sm text-gray-700 resize-none"></textarea>
                                </div>
                                <div class="mt-6 flex gap-3">
                                    <button type="submit" class="w-full bg-teal-600 hover:bg-teal-700 text-white rounded-xl py-3 font-black transition active:scale-95">Accept & Create Contract</button>
            ...
            {{-- 2. Dynamic Reschedule Modal (With Price Math!) --}}
            <div x-data="{ 
                ...
                <div x-show="showReschedule" x-transition class="fixed inset-0 z-10 w-screen overflow-y-auto">
                    <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                        <div @click.away="showReschedule = false" class="relative transform overflow-hidden rounded-[2rem] bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg border border-gray-100 p-8">
                            ...
                            <div class="bg-gray-50 p-4 rounded-xl mb-4 border border-gray-200 flex justify-between items-center mt-4">
                                <div>
                            ...
                            <form action="{{ route('owner.reservations.reschedule', $rentRequest->id) }}" method="POST">
                                ...
                                <div>
                                    <label class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-2">Reason / Message</label>
                                    <textarea name="response_note" rows="3" placeholder="Hi, I am unavailable on your requested dates. Could we do this instead?" class="w-full rounded-2xl border-gray-300 focus:border-blue-500 shadow-sm font-medium text-sm text-gray-700 resize-none"></textarea>
                                </div>
                                <div class="mt-6 flex gap-3">
                        ...
                        <div @click.away="showDecline = false" class="relative transform overflow-hidden rounded-[2rem] bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg border border-gray-100 p-8">
                            ...
                            <form action="{{ route('owner.reservations.reject', $rentRequest->id) }}" method="POST" class="mt-4">
                                ...
                                <div>
                                    ...
                                    <textarea name="reject_reason" required rows="3" placeholder="I'm sorry, but the space is currently undergoing maintenance..." class="w-full rounded-2xl border-gray-300 focus:border-red-500 shadow-sm font-medium text-sm text-gray-700 resize-none"></textarea>
                                </div>
                                <div class="mt-6 flex gap-3">
                                    <button type="submit" class="w-full bg-red-600 hover:bg-red-700 text-white rounded-xl py-3 font-black transition active:scale-95">Decline Application</button>
