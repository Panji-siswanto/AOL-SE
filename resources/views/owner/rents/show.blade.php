<x-user-layout>
    ...
    <div class="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        ...
        {{-- The Magic 'items-start' enables the sticky right column --}}
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 xl:gap-12 items-start">
            
            {{-- Main Content (Left Column) --}}
            <div class="lg:col-span-2 space-y-8">
                
                {{-- Space Summary (Layout Fixed with CSS Grid) --}}
                ...
                <div class="bg-white p-8 rounded-[2rem] border border-gray-100 shadow-sm">
                    ...
                    @php
                        $proposalMsg = $rentRequest->messages->where('type_id', \App\Models\Status::MSG_PROPOSAL)->first();
                    @endphp

                    @if($proposalMsg)
                        <div class="bg-gray-50 p-6 rounded-2xl border border-gray-200">
                            <p class="text-gray-700 font-medium leading-relaxed whitespace-pre-line">{{ $proposalMsg->note }}</p>
                        </div>
                    @else
                        <div class="bg-gray-50 p-6 rounded-2xl border border-gray-200 text-center text-gray-400 font-medium">
            ...
            <div class="lg:col-span-1 sticky top-28 bg-white p-6 sm:p-8 rounded-[2rem] border border-gray-200 shadow-xl shadow-gray-100/50">
                ...
                <div class="space-y-4 mb-8">
                    ...
                    <div class="flex justify-between items-center">
                        ...
                        <span class="text-sm font-black text-gray-900">
                            @if($rentRequest->pricing && $rentRequest->duration)
                                {{ $rentRequest->duration }} {{ \Illuminate\Support\Str::plural($rentRequest->duration_unit, $rentRequest->duration) }}
                            @else
                                N/A
                            @endif
                        </span>
                ...
                {{-- Action Buttons --}}
                @if($rentRequest->status_id == \App\Models\Status::RNT_REQ_PENDING)
                    <div x-data="{ showApprove: false, showReschedule: false }" x-cloak class="space-y-3 pt-6 border-t border-gray-100">
                        <button type="button" @click.prevent="showApprove = true" class="w-full bg-teal-600 hover:bg-teal-700 text-white py-3.5 rounded-2xl font-black transition-all active:scale-95 shadow-lg shadow-teal-600/30">
                            Accept Application
                        </button>
                        <button type="button" @click.prevent="showReschedule = true" class="w-full bg-blue-50 hover:bg-blue-100 text-blue-700 border border-blue-200 py-3.5 rounded-2xl font-black transition-all active:scale-95 shadow-sm">
                            Propose New Visit Date
                        </button>
                        <form action="{{ route('owner.reservations.reject', $rentRequest->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to REJECT this application? This cannot be undone.');">
                            @csrf
                            <button type="submit" class="w-full bg-white hover:bg-red-50 text-red-500 hover:text-red-600 border-2 border-red-100 hover:border-red-200 py-3.5 rounded-2xl font-black transition-all active:scale-95">
                                Decline Application
                            </button>
                        </form>
