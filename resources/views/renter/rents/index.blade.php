    ...
    <div class="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8" x-data>
        ...
        <div class="mb-8">
            ...
            <div class="flex space-x-1 bg-gray-100/80 p-1 rounded-2xl w-full sm:w-fit mb-6 overflow-x-auto scrollbar-hide">
                <a href="{{ route('rents.index', ['status' => 'all', 'sort' => $currentSort, 'search' => request('search')]) }}" 
                   class="px-6 py-2.5 text-sm font-bold rounded-xl transition-all whitespace-nowrap {{ $currentStatus === 'all' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-200/50' }}">
                    All Requests
                </a>
                <a href="{{ route('rents.index', ['status' => 'rnt_req_pending', 'sort' => $currentSort, 'search' => request('search')]) }}" 
                   class="px-6 py-2.5 text-sm font-bold rounded-xl transition-all whitespace-nowrap {{ $currentStatus === 'rnt_req_pending' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-200/50' }}">
                    Pending
                </a>
                <a href="{{ route('rents.index', ['status' => 'rnt_req_rejected', 'sort' => $currentSort, 'search' => request('search')]) }}" 
                   class="px-6 py-2.5 text-sm font-bold rounded-xl transition-all whitespace-nowrap {{ $currentStatus === 'rnt_req_rejected' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-200/50' }}">
                    Rejected
                </a>
                <a href="{{ route('rents.index', ['status' => 'rnt_req_cancelled', 'sort' => $currentSort, 'search' => request('search')]) }}" 
                   class="px-6 py-2.5 text-sm font-bold rounded-xl transition-all whitespace-nowrap {{ $currentStatus === 'rnt_req_cancelled' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-200/50' }}">
                    Cancelled
                </a>
                <a href="{{ route('rents.index', ['status' => 'rnt_ongoing', 'sort' => $currentSort, 'search' => request('search')]) }}" 
                class="px-6 py-2.5 text-sm font-bold rounded-xl transition-all whitespace-nowrap {{ $currentStatus === 'rnt_ongoing' ? 'bg-blue-500 text-white shadow-sm shadow-blue-500/30' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-200/50' }}">
                    Ongoing (Active)
                </a>
                <a href="{{ route('rents.index', ['status' => 'rnt_completed', 'sort' => $currentSort, 'search' => request('search')]) }}" 
                class="px-6 py-2.5 text-sm font-bold rounded-xl transition-all whitespace-nowrap {{ $currentStatus === 'rnt_completed' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-200/50' }}">
                    History (Completed)
                </a>
            </div>
        ...
        @if($requests->count() > 0)
            <div class="space-y-4">
                @foreach($requests as $request)
                        ...
                        <div class="flex-1 min-w-0 w-full flex flex-col justify-between h-full">
                            <div class="flex justify-between items-start gap-4 mb-3">
                                ...
                                <div class="flex-shrink-0">
                                    ...
                                    @elseif($waitingForOwner || $waitingForOwnerFinish) <span class="bg-blue-50 text-blue-600 px-3 py-1.5 rounded-lg text-[10px] font-black border border-blue-100 uppercase tracking-wider">Waiting for Owner</span>
                            ...
                            <div class="flex justify-between items-center">
                                ...
                                <button @click.prevent="$dispatch('open-details-modal-{{ $request->id }}')" class="{{ ($isMyTurn || $isMyTurnFinish) ? 'bg-orange-500 hover:bg-orange-600 shadow-orange-500/20 text-white' : 'bg-gray-900 hover:bg-black shadow-gray-900/20 text-white' }} px-5 py-2 rounded-xl text-xs font-black transition-all shadow-md active:scale-95">
                    ...
                    {{-- 1. MASTER DETAILS MODAL (REDESIGNED) --}}
                                            ...
                                            <div class="space-y-3 mt-4">
                                                ...
                                                @if($request->status_id == $ongoingId)
                                                    ...
                                                    @elseif($waitingForOwnerFinish)
                                                        <button class="w-full bg-gray-100 text-gray-500 py-3.5 rounded-2xl font-bold text-sm cursor-not-allowed" disabled>Waiting for Response...</button>
                                                    @else
                    ...
                    {{-- 🔥 EARLY FINISH ONGOING MODALS (Renter side) --}}
                    @if($request->status_id == $ongoingId)
                        
                        {{-- Renter Requesting Early Finish --}}
                        @if(!$pendingFinishMsg)
                        ...
                        {{-- Renter Approving Owner's Early Finish Request --}}
                        @if($isMyTurnFinish)
                        ...
                        {{-- Renter Rejecting Owner's Early Finish Request --}}
