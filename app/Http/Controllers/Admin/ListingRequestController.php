...
class ListingRequestController extends Controller {
    ...
    public function index(Request $request) {
        $query = SpaceRegistration::with(['location', 'status', 'owner', 'documents.documentType', 'prices.pricingType'])
            ->where('status_id', $this->getStatusId('reg_pending'))
    ...
    public function history(Request $request) {
        $query = SpaceRegistration::with(['location', 'status', 'owner', 'documents.documentType', 'prices.pricingType', 'logs.admin'])
            ->whereIn('status_id', [$this->getStatusId('reg_approved'), $this->getStatusId('reg_rejected')])
            ->search($request->search)
    ...
    public function approve(Request $request, SpaceRegistration $registration) {
        if ($registration->owner->ver_status !== $this->getStatusId('usr_verified')) {
            return redirect()->back()->with('error', 'Cannot approve listing. The host\'s identity (KTP) must be verified first.');
        }

        DB::beginTransaction();
        try {
            $registration->update(['status_id' => $this->getStatusId('reg_approved')]);
            ...
            $space = Space::create([
                ...
                'price'=> $registration->prices()->min('price') ?? 0,
                'status_id'=> $this->getStatusId('spc_available'),
            ]);
            ...
            DB::commit();
            return redirect()->back()->with('success', "Space listing '{$space->name}' has been approved.");
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    public function reject(Request $request, SpaceRegistration $registration) {
        ...
        DB::beginTransaction();
        try {
            $registration->update(['status_id' => $this->getStatusId('reg_rejected')]);
    ...
    private function getStatusId($code) {
